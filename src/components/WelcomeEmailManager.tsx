...
import { Mail, Settings, BarChart3, Users, ToggleLeft, ToggleRight, Save, Send } from 'lucide-react';
...
export default function WelcomeEmailManager() {
  ...
  const loadSettings = async () => {
    try {
      ...
      setSettings({
        welcome_email_enabled: isEnabled,
    ...
    try {
      if (state.settings?.id) {
        await actions.updateSettings(state.settings.id, {
          ...
          welcome_email_template_id: settings.welcome_email_template_id || undefined
        });
  ...
  const sendTestEmail = async () => {
    ...
    try {
      const template = state.emailTemplates.find(t => t.id === settings.welcome_email_template_id);
      if (!template) {
    ...
    const testCustomer = {
      ...
      const toggleWelcomeEmail = () => {
        setSettings(prev => ({
          ...prev,
          welcome_email_enabled: !prev.welcome_email_enabled
        }));
      };
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-3xl font-bold ${settings.welcome_email_enabled ? 'text-green-600' : 'text-red-600'}`}>
                    {settings.welcome_email_enabled ? 'ON' : 'OFF'}
                  </p>
                </div>
                <div className={`p-3 rounded-lg ${settings.welcome_email_enabled ? 'bg-green-50' : 'bg-red-50'}`}>
                  <Settings className={`h-6 w-6 ${settings.welcome_email_enabled ? 'text-green-600' : 'text-red-600'}`} />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              {settings.welcome_email_enabled && (
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={settings.welcome_email_template_id}
                      onChange={(e) => setSettings(prev => ({ ...prev, welcome_email_template_id: e.target.value }))}
                  ...
                  {settings.welcome_email_template_id && (
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-2">Template Preview</h4>
                      {(() => {
                        const template = state.emailTemplates.find((t: EmailTemplate) => t.id === settings.welcome_email_template_id);
                        return template ? (
              ...
              <div className="flex justify-end space-x-3">
                <button
                  ...
                  disabled={!settings.welcome_email_enabled || !settings.welcome_email_template_id || testingEmail}
