...
import { UserProfile, CustomerPortalUser } from '../types';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  }, [loadUserData, authInitialized]); // Removed userProfile to prevent infinite loop
  ...
  const signOut = useCallback(async (skipRedirect?: boolean) => {
    setAuthLoading(true);
    setError(null);
    ...
    try {
      ...
      // This is critical because onAuthStateChange might be delayed or missed in some race conditions
      setAuthUser(null);
      setUserProfile(null);
      setIsAdmin(false);
      setCustomerPortalUser(null);
